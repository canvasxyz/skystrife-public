...
libp2p.services.discovery.addEventListener("peer:topics", ({ detail: { topics, isUniversalReplication, peerId } }) => {
  ...
  for (const topic of topics) {
    ...
    if (bannedApps.has(appTopic)) {
      // console.error(`[replication-server] Skipped banned app ${appTopic}`);
      continue;
    }
    ...
    if (apps.size > maxTopics && !apps.has(appTopic)) {
      console.error(`[replication-server] Received topic ${appTopic} but over max topics: ${apps.size}/${maxTopics}`);
      return;
    }
...
//
// terminate apps after sky strife matches end
//
...
defineEnterSystem(world, [Has(MatchConfig), Has(MatchFinished)], ({ entity }) => {
  ...
  bannedApps.add(appTopic);

  const app = apps.get(appTopic);
  if (app !== undefined) {
    console.log(`[replication-server] Stopping app ${appTopic} after match finished`);
    apps.delete(appTopic);
    lastActive.delete(appTopic);
    app.close();
  } else {
    console.log(`[replication-server] Banned app ${appTopic} after match finished`);
  }
});

//
// terminate apps after prolonged inactivity
// (doesn't work if browser windows remain open)
//
